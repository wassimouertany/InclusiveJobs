...
// @ts-ignore: side-effect font import without type declarations
...
const BIG_CURSOR_SVG =
  '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"><path fill="%23000" stroke="%23fff" stroke-width="1.5" d="M6 2v20l6-5 4 7 2-1.5-4-6.5 10-.5z"/></svg>';
const BIG_CURSOR_DATA_URI =
  "data:image/svg+xml," + encodeURIComponent(BIG_CURSOR_SVG);
...
export default function AccessibilityWidget() {
  ...
  useEffect(() => {
    ...
    el.textContent =
      "body { font-size: calc(1rem * var(--a11y-font-scale, 1)) !important; line-height: var(--a11y-line-height, 1.5) !important; }";
    return () => {
      removeStyle(STYLE_IDS.bodyVars);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    document.body.style.setProperty(
      "--a11y-line-height",
      String(LINE_HEIGHT_VALUES[lineHeightLevel]),
    );
    document.body.style.setProperty(
      "letter-spacing",
      LETTER_SPACING_VALUES[letterSpacingLevel],
    );
  ...
  useEffect(() => {
    if (!readableFont) {
      removeStyle(STYLE_IDS.readableFont);
      return;
    }
    const el = getOrCreateStyle(STYLE_IDS.readableFont);
    el.textContent =
      "body, body * { font-family: 'OpenDyslexic', sans-serif !important; }";
  }, [readableFont]);
  ...
  useEffect(() => {
    ...
    el.textContent =
      '* { cursor: url("' + BIG_CURSOR_DATA_URI + '") 16 0, auto !important; }';
  }, [bigCursor]);
  ...
  useEffect(() => {
    ...
    if (highContrast) {
      ...
      el.textContent =
        "body { background: #000 !important; color: #fff !important; } a { color: #ff0 !important; } button, input { background: #111 !important; border: 2px solid #fff !important; }";
    }
    if (monochrome) {
  ...
  useEffect(() => {
    ...
    el.textContent =
      "img, picture, figure, svg, video { visibility: hidden !important; }";
  }, [hideImages]);
  ...
  useEffect(() => {
    ...
    el.textContent =
      "p:hover, li:hover, div:hover, h1:hover, h2:hover, h3:hover, h4:hover, h5:hover, h6:hover { outline: 2px solid #4F46E5 !important; background: rgba(79,70,229,0.08) !important; }";
  }, [highlightContent]);
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                    ...
                    <div className="flex items-center justify-between">
                      <button
                        onClick={() =>
                          setLineHeightLevel((n) => Math.max(0, n - 1))
                        }
                        className="p-2 text-indigo-600 hover:bg-indigo-100 rounded-lg transition-colors"
                      >
                        <MinusCircle className="w-5 h-5" />
                      ...
                      <span className="text-sm font-semibold text-gray-900 w-20 text-center">
                        {LINE_HEIGHT_LEVELS[lineHeightLevel]}
                      </span>
                      <button
                        onClick={() =>
                          setLineHeightLevel((n) => Math.min(3, n + 1))
                        }
                        className="p-2 text-indigo-600 hover:bg-indigo-100 rounded-lg transition-colors"
                      >
                        <PlusCircle className="w-5 h-5" />
                  ...
                  <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                    ...
                    <div className="flex items-center justify-between">
                      <button
                        onClick={() =>
                          setLetterSpacingLevel((n) => Math.max(0, n - 1))
                        }
                        className="p-2 text-indigo-600 hover:bg-indigo-100 rounded-lg transition-colors"
                      >
                        <MinusCircle className="w-5 h-5" />
                      ...
                      <span className="text-sm font-semibold text-gray-900 w-20 text-center">
                        {LETTER_SPACING_LEVELS[letterSpacingLevel]}
                      </span>
                      <button
                        onClick={() =>
                          setLetterSpacingLevel((n) => Math.min(3, n + 1))
                        }
                        className="p-2 text-indigo-600 hover:bg-indigo-100 rounded-lg transition-colors"
                      >
                        <PlusCircle className="w-5 h-5" />
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button
                      onClick={() =>
                        cycle(ALIGN_OPTIONS, alignIndex, setAlignIndex)
                      }
                    ...
                    >
                      ...
                      <span className="text-xs font-medium">
                        Align: {ALIGN_OPTIONS[alignIndex]}
                      </span>
                    </button>
                    <button
                      onClick={() =>
                    ...
                    >
                      ...
                      <span className="text-xs font-medium">
                        Weight: {FONT_WEIGHT_OPTIONS[fontWeightIndex]}
                      </span>
                    </button>
                  </div>
                </div>
